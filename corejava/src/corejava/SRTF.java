...
public class SRTF {
    public static void srtfScheduling(List<Process> processes) {
        ...
        while (completed < n) {
            ...
            if (currentProcess == null || (!readyQueue.isEmpty() && readyQueue.stream().min(Comparator.comparingInt(p -> p.remainingTime)).get().remainingTime < currentProcess.remainingTime)) {
                if (currentProcess != null && currentProcess.remainingTime > 0) {
